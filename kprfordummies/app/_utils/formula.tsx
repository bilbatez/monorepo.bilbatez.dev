...
function generateAmortizationTableWithFlatInterestRate(
  ...
  for (let index = 0; index < request.interestPeriod.length; index++) {
    ...
    const totalInterestPeriod: number =
      initialLoanBalance *
      (request.interestPeriod[0].interestRate / 100) *
      (totalMonths / 12);
    ...
    for (let index = 0; index < totalMonthsPeriod; index++) {
      ...
      const finalLoanBalance =
        initialLoanBalance - monthlyRepayment < 0
          ? 0
          : initialLoanBalance - monthlyRepayment;
...
function generateAmortizationTableWithEffectiveInterestRate(
  ...
  for (let index = 0; index < request.interestPeriod.length; index++) {
    ...
    const monthlyInterestRate =
      request.interestPeriod[index].interestRate / 12 / 100;
